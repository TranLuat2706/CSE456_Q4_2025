...
public class StudentController {
    ...
    * 2. Thông qua nút bấm search*/
    @GetMapping("students")
    public String students(@RequestParam(value = "keyword", defaultValue = "") String keyword, Model model, HttpSession ses) {
        User user = (User) ses.getAttribute("user");
        if (user == null) {
            return "redirect:/login"; // chưa login thì về trang login
        }
    ...
    public String editStudent(@PathVariable("id") String id, Model model, HttpSession ses,
        ...
        if(user.getRole()!=1 && user.getRole()!=2){
            RedAttr.addFlashAttribute("errRole", "Access Denied");
